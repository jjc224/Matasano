...
def aes_cbc_decrypt(enc, key, iv = "\0" * 16)    # 16 byte blocks (128-bit cipher).
	...
	dec_block  = aes_ecb_decrypt(enc_blocks[0], key)
	plaintext  = [xor_encrypt(dec_block, iv)].pack('H*')
	prev_block = enc_blocks[0]

	# Neglect the first block and iterate through the rest.
	enc_blocks.shift
	enc_blocks.each_with_index do |block, i|
		dec_block = aes_ecb_decrypt(enc_blocks[i], key)
		plaintext << [xor_encrypt(dec_block, prev_block)].pack('H*')
...
enc = Base64.decode64(open('http://cryptopals.com/static/challenge-data/10.txt') { |f| f.read }.strip!)
