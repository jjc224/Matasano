...
# Transpose each ciphertext C from the array into its own respective column (i.e. select every k'th character from every block of C).
# Return the transposed array.
def transpose(ciphertexts)
    columns = [ciphertexts[0][0]].zip(*ciphertexts[1..-1].map(&:chars))

#    columns = [[] * ciphertexts.size]
#
#    ciphertexts.each_with_index do |c, i|
#        ciphertexts.map { |c| columns[i] << c[i] || '' }.join
#    end
end
...
def decrypt_next_byte(ciphertexts, block_i)
    column = ciphertexts.map { |c| c[block_i] || '' }.join
...
def decrypt_keystream(ciphertexts)
    ...
    ciphertexts.map.with_index do |c, i|
        candidate_key = decrypt_next_byte(ciphertexts, i)       # Do '(c.size).times' if necessary (code seems to be highly robust and derives the same, correct key for every instance.
